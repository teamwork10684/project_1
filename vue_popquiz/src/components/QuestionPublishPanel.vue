      ...
      <div class="panel-list">
        <div class="button-area">
          ...
          <a-button
            ...
            @click="isAutoPublishing ? stopAutoPublish() : startAutoPublish()"
          >
            <template v-if="isAutoPublishing">
              <LoadingOutlined spin style="margin-right:6px;vertical-align:middle;" />
              暂停
            </template>
            <template v-else>
              自动发布
        ...
        <div class="question-list">
          <div v-if="questions.length === 0" class="empty-state">
            ...
            <div class="empty-text">
              ...
              <div class="empty-description">等待后台生成题目中...</div>
            </div>
          </div>
          <div 
            v-else
...
const emit = defineEmits(['publishQuestion', 'saveSettings']);
...
const startAutoPublish = () => {
  if (isAutoPublishing.value) return;
...
const scheduleAutoPublish = () => {
  ...
  autoPublishTimer = setTimeout(() => {
    autoPublishOnce();
    scheduleAutoPublish();
  }, autoPublishInterval.value * 60 * 1000);
};
const autoPublishOnce = () => {
  // 找到第一个未发布的题目
...
watch(
  ...
  (newQuestions) => {
    if (selectedQuestion.value) {
      const updated = newQuestions.find(q => q.id === selectedQuestion.value.id);
      if (updated) {
